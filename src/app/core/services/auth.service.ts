...
export class AuthService {
	...
	loadAuthUser() {
		if (this.rememberMe) {
			if (localStorage.getItem(environment.authUserKey)) {
				this.#authUser.set(
					JSON.parse(<string>localStorage.getItem(environment.authUserKey))
				);
			} else {
			...
			if (sessionStorage.getItem(environment.authUserKey)) {
				this.#authUser.set(
					JSON.parse(<string>sessionStorage.getItem(environment.authUserKey))
				);
			} else {
				this.#authUser.set(null);
			}
	...
	loadJWT() {
		if (this.rememberMe) {
			...
			if (sessionStorage.getItem(environment.authJWTKey)) {
				this.#authJWT.set(
					<string>sessionStorage.getItem(environment.authJWTKey)
				);
			} else {
				this.#authJWT.set(null);
			}
	...
	loadRefresh() {
		if (this.rememberMe) {
			if (localStorage.getItem(environment.authRefreshKey)) {
				this.#authRefresh.set(
					<string>localStorage.getItem(environment.authRefreshKey)
				);
			} else {
			...
			if (sessionStorage.getItem(environment.authRefreshKey)) {
				this.#authRefresh.set(
					<string>sessionStorage.getItem(environment.authRefreshKey)
				);
			} else {
				this.#authRefresh.set(null);
			}
	...
	logout(makeRequest = true) {
		...
		if (this.isAuth && makeRequest) {
			this.#http
				.get(`${environment.apiUrl}${ApiEndpoit.logoutURL}`)
				.subscribe(removeCredentials);
		} else {
			removeCredentials();
		}
	}
	...
	verifyToken(subject: Subject<boolean>, token: string) {
		return this.#http.post(
			`${environment.apiUrl}${ApiEndpoit.verifyTokenURL}`,
			{ token }
		);
	}
